...
def add_delete(driver, df, existing_skills):
    for i, row in df.iterrows():
        ...
        try:
            driver.find_element(By.XPATH,'//input[@class="MuiInputBase-input MuiInput-input MuiInputBase-inputAdornedStart"]').send_keys(row['name'])
            driver.find_element(By.XPATH,'//input[@class="MuiInputBase-input MuiInput-input MuiInputBase-inputAdornedStart"]').send_keys(Keys.ENTER)
            driver.find_element(By.XPATH, "//span[text()='{}']/ancestor::div[@role='button']".format(row['name'])).click()
        except Exception:
            pass
        try:
